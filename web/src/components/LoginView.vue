...
<template>
  <main class="login-shell"><section class="login-card"><p class="eyebrow">RELAY · SELF-HOSTED</p><h1>PromptJang <span>Relay</span></h1><p class="muted">Reliable webhook delivery, on your PostgreSQL.</p><form @submit.prevent="submit"><label>Username<input v-model="form.username" type="text" required autocomplete="username"></label><label>Password<input v-model="form.password" type="password" required autocomplete="current-password"></label><p v-if="error && submitted" class="error" role="alert">{{ error }}</p><button :disabled="loading">{{ loading ? 'Signing in…' : 'Sign in' }}</button></form></section></main>
</template>
