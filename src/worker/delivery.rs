...
pub(crate) async fn process_one(pool: &PgPool, config: &Config) -> Result<bool> {
...
async fn succeed(
    ...
    sqlx::query(
        "UPDATE events SET status='DELIVERED',last_error=NULL,updated_at=now() WHERE id=$1",
    )
    .bind(job.id)
    .execute(&mut *tx)
    .await?;
...
pub(crate) async fn recover_stuck(pool: &PgPool, config: &Config) -> Result<u64> {
    let recovered = sqlx::query("UPDATE events SET status='RETRYING',next_attempt_at=now(),last_error='recovered after interrupted delivery',updated_at=now() WHERE status='PROCESSING' AND updated_at < now()-make_interval(secs=>$1)")
